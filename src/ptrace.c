...
void ptrace_attach(pid_t target)
{
  ...
  dzlog_debug("Attaching to PID %i", target);
  int waitpidstatus;

	if(ptrace(PTRACE_ATTACH, target, NULL, NULL) == -1)
	{
		dzlog_fatal("ptrace(PTRACE_ATTACH) failed");
		exit(1);
	}

	if(waitpid(target, &waitpidstatus, WUNTRACED) != target)
	{
		dzlog_fatal("waitpid(%d) failed", target);
		exit(1);
	}
}
...
{
  dzlog_debug("Detaching from PID %i", target);
	if(ptrace(PTRACE_DETACH, target, NULL, NULL) == -1)
	{
	        dzlog_fatal("ptrace(PTRACE_DETACH) failed");
		exit(1);
	}
}
...
void ptrace_getregs(pid_t target, struct user_regs_struct* regs)
{
  ...
  dzlog_debug("Getting registers of PID %i", target);
  if(ptrace(PTRACE_GETREGS, target, NULL, regs) == -1)
	{
		dzlog_fatal("ptrace(PTRACE_GETREGS) failed");
		exit(1);
	}
}

void ptrace_just_cont(pid_t target){
  dzlog_debug("Continuing PID %i", target);

  if(ptrace(PTRACE_CONT, target, NULL, NULL) == -1)
	{
	        dzlog_fatal("ptrace(PTRACE_CONT) failed");
		exit(1);
       }
}
...
{
  dzlog_debug("Setting registers of PID %i", target);
  if(ptrace(PTRACE_SETREGS, target, NULL, regs) == -1)
	{
		dzlog_fatal("ptrace(PTRACE_SETREGS) failed");
		exit(1);
	}
}
...
{
        dzlog_debug("Getting signal info for PID %i", target);
        siginfo_t targetsig;
	if(ptrace(PTRACE_GETSIGINFO, target, NULL, &targetsig) == -1)
	{
		dzlog_fatal("ptrace(PTRACE_GETSIGINFO) failed");
		exit(1);
	}
	return targetsig;
}
...
{
        dzlog_debug("Reading %i bytes from 0x%lx in PID %i", len, addr, pid);
        int bytesRead = 0;
	int i = 0;
	long word = 0;
	long *ptr = (long *) vptr;
	...
	while (bytesRead < len)
	{
		...
		if(word == -1)
		{
			dzlog_fatal("ptrace(PTRACE_PEEKTEXT) failed");
			exit(1);
		}
...
{
  dzlog_debug("Writing %i bytes to 0x%lx in PID %i", len, addr, pid);
        int byteCount = 0;
	long word = 0;
	...
	while (byteCount < len)
	{
		...
		if(word == -1)
		{
			dzlog_fatal("ptrace(PTRACE_POKETEXT) failed");
			exit(1);
		}
...
void checktargetsig(int pid)
{
	...
	{
		dzlog_fatal("instead of expected SIGTRAP, target stopped with signal %d: %s", targetsig.si_signo, strsignal(targetsig.si_signo));
		dzlog_fatal("sending process %d a SIGSTOP signal for debugging purposes", pid);
