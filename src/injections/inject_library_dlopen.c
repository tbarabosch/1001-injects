#include "inject_library_dlopen.h"
...
void inject_library_dlopen(pid_t target, char* libname){
        dzlog_debug("Injecting library %s in PID %i via dlopen", libname, target);
	...
	long freeAddr = getFunctionAddress("free", (char*)&"libc.so.6");
	long dlopenAddr = getFunctionAddress("__libc_dlopen_mode", (char*)&"libc.so.6");
	...
	long targetLibcAddr = getlibaddr(target, (char*)&"libc-");
	...
	if(libAddr == 0)
	{
	        dzlog_error("__libc_dlopen_mode() failed to load %s", libname);
		restoreStateAndDetach(target, addr, backup, injectSharedLibrary_size, oldregs);
		free(backup);
		free(newcode);
		exit(1);
	...
	{
		dzlog_info("\"%s\" successfully injected", libname);
	}
	else
	{
	        dzlog_error("could not inject \"%s\"", libname);
	}
